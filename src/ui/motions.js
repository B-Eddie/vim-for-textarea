  ...
  function buildRow(section, item, idx) {
    ...
    cols.forEach(col => {
      ...
      if (col.type === 'readonly') {
        ...
        const text = document.createElement('div'); text.style.minHeight = '24px'; text.style.fontFamily = 'ui-monospace, monospace'; text.style.fontSize = '12px'; text.style.color = '#444';
        text.textContent = tokensToStr(item[col.key]);
        const btn = document.createElement('button'); btn.textContent = 'Edit'; btn.style.position = 'absolute'; btn.style.right = '48px'; btn.style.top = '0'; btn.style.background='transparent'; btn.style.border='none'; btn.style.color='var(--primary-color)'; btn.style.padding='0'; btn.style.fontSize='12px'; btn.style.cursor='pointer';
        btn.addEventListener('click', () => { openCommandModal(item, () => { onConfigChange(); text.textContent = tokensToStr(item[col.key]); }); });
        const remove = document.createElement('button'); remove.textContent = 'Remove'; remove.style.position = 'absolute'; remove.style.right = '0'; remove.style.top = '0'; remove.style.background='transparent'; remove.style.border='none'; remove.style.color='#b00020'; remove.style.padding='0'; remove.style.fontSize='12px'; remove.style.cursor='pointer';
        remove.title = 'Remove this command';
        remove.addEventListener('click', () => {
          const list = currentConfig.commands || [];
          const index = list.indexOf(item);
          if (index !== -1) list.splice(index, 1);
          onConfigChange();
          buildEditor();
        });
        cont.appendChild(text); cont.appendChild(btn); cont.appendChild(remove);
        input = cont;
      }
      td.appendChild(input);
      tr.appendChild(td);
    });
    // Command rows can be removed from the configuration.
    return tr;
  }
